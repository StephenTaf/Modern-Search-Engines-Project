...
try:
    ...
    print("Warning: Multiprocessing support not available")
...
class TuebingenCrawler:
    ...
    def _setup_signal_handlers(self):
        ...
        def signal_handler(signum, frame):
            ...
            # Give a bit of time for current operations to complete
    ...
    def crawl_url(self, url: str, entry) -> bool:
        ...
        try:
            ...
            # Check if URL is allowed using fast robots.txt checking
    ...
    def _process_html_content(self, url: str, response, entry) -> bool:
        ...
        try:
            # Extract text content using fast method for consistent performance
            content_type = self.http_client.get_content_type(response) or "text/html"
            content_size = len(response.text)
            
            # Use fast text extraction for all content sizes
            ...
            # Extract and add new URLs to frontier using fast method
            # Use content size limits similar to multiprocessing version
            url_extraction_content = response.text
            if content_size > 2*1024 * 1024:  # 500KB
    ...
    def start(self, seed_urls: Optional[List[str]] = None):
        ...
        # Check multiprocessing availability and configuration
        if self.config.enable_multiprocessing and not MULTIPROCESSING_AVAILABLE:
            print("Warning: Multiprocessing requested but not available, falling back to single-threaded mode")
            self.config.enable_multiprocessing = False
        
        # Use provided seed URLs or default ones
        if seed_urls:
    ...
    def _start_multiprocessing_crawler(self):
        ...
        try:
            # Create multiprocessing crawler
